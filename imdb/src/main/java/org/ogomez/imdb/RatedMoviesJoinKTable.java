...
import org.ogomez.imdb.serializers.MovieCustomSerdes;
...
public class RatedMoviesJoinKTable {
  ...
  public static void main(String[] args) {
    ...
    KStream<String, Rating> ratingsStream = builder.stream(MovieTopology.RATINGS_TOPIC,
        Consumed.with(Serdes.String(), MovieCustomSerdes.Rating()))
        .map((key, rating) ->
            new KeyValue<>(rating.getPayload().getTconst(), rating));

    ratingsStream.to(MovieTopology.REKEYED_RATING_TOPIC,
        Produced.with(Serdes.String(), MovieCustomSerdes.Rating()));

    KStream<String, Rating> ratings = builder.stream(MovieTopology.REKEYED_RATING_TOPIC,
        Consumed.with(Serdes.String(), MovieCustomSerdes.Rating()));
    ...
    final KafkaStreams streams = new KafkaStreams(topology,
        MovieTopology.createStreamsConfigProperties("imdb"));
    streams.cleanUp();
